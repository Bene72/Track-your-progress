...
import { PERSONAL_RECORD_COLUMNS, PERSONAL_SESSION_COLUMNS } from '../db-columns'
...
export function usePrData(userId) {
  ...
  const loadAll = useCallback(async () => {
    ...
    const [recRes, sessRes, latestRes] = await Promise.all([
      supabase.from('personal_records').select(PERSONAL_RECORD_COLUMNS).eq('user_id', userId).order('achieved_at', { ascending: false }),
      supabase.from('personal_sessions').select(PERSONAL_SESSION_COLUMNS).eq('user_id', userId).order('session_date', { ascending: false }),
      supabase.from('personal_records_latest').select('user_id, movement, value_type, value_number, achieved_at, notes').eq('user_id', userId),
    ])
