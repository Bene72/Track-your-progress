...
export function usePersonalTraining(userId) {
  ...
  // NOUVELLE FONCTION : Supprimer une séance entière
  ...
  }, [userId, fetchWeeklyVolume])

  const addExercise = useCallback(async (sessionId, exerciseId) => {
    const session = sessions.find(s => s.id === sessionId)
    const { data, error } = await supabase
      .from('personal_session_exercises')
      .insert({ session_id: sessionId, exercise_id: exerciseId, position: session?.exercises.length || 0 })
  ...
  // NOUVELLE FONCTION : Ajouter un exercice en superset (même position)
  ...
  // NOUVELLE FONCTION : Changer la position d'un exercice (pour créer des supersets)
  ...
  return {
    ...
    weeklyVolume,
    createSession,
    deleteSession, // NOUVEAU
    addExercise,
    addExerciseToSuperset, // NOUVEAU
    moveExercise, // NOUVEAU
    addCustomExercise,
