...
export default function BoxPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    // !left force un LEFT JOIN vers profiles : sans ça, la RLS de
    // `profiles` (chacun ne lit que son propre profil) fait disparaître
    // en INNER JOIN les lignes des autres membres, et "Adhérents"
    // affiche 0 même quand box_members contient bien des lignes actives.
    const [{ data: m }, { data: inv }] = await Promise.all([
      supabase.from('box_members').select('*, profiles!left ( full_name )').eq('box_id', box.activeBoxId).eq('status', 'active'),
      box.isCoach
