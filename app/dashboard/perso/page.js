...
export default function PersonalTrainingPage() {
  ...
  const appliedDateParam = useRef(null)

  // Lien direct depuis "Aujourd'hui" (?date=YYYY-MM-DD) : bascule sur ce
  // jour-là dès que les données sont prêtes. Un seul passage par valeur de
  // paramètre (via le ref) pour ne pas re-forcer la date si l'utilisateur
  // navigue ensuite manuellement vers un autre jour sur cette même page.
  useEffect(() => {
    const dateParam = searchParams.get('date')
    if (dateParam && dateParam !== appliedDateParam.current && !pt.loading) {
      appliedDateParam.current = dateParam
      pt.changeDate(dateParam)
    }
  }, [searchParams, pt])
              ...
              {pt.sessions.map(session => (
                <div key={session.id} className="session-block">
                  ...
                  <PersonalSessionCard
                    ...
                    onFetchLastPerformance={pt.getLastPerformance}
                    onReorderBlocks={pt.reorderBlocks}
                    onDuplicateSession={pt.duplicateSession}
                    templates={pt.templates}
                    onSaveAsTemplate={pt.saveSessionAsTemplate}
                    onApplyTemplate={pt.applyTemplateToSession}
                  />
                </div>
              ))}
            </div>
