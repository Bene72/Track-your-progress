...
import { estimate1RM } from '../../lib/fitness-math'
...
export function AddToBlockInline({ blockId, catalog, onAddExerciseToBlock, onAddCustomExercise, onFetchLastPerformance }) {
  ...
  const [lastPerfHint, setLastPerfHint] = useState(null)

  // Dès qu'un mouvement existant est choisi, on va chercher la dernière
  // performance connue et on pré-remplit les champs (l'utilisateur garde la
  // main pour corriger avant d'ajouter).
  useEffect(() => {
    let cancelled = false
    async function prefill() {
      if (!exerciseId || !onFetchLastPerformance) { setLastPerfHint(null); return }
      const last = await onFetchLastPerformance(exerciseId)
      if (cancelled || !last) { setLastPerfHint(null); return }
      setReps(prev => prev || (last.reps != null ? String(last.reps) : ''))
      setWeight(prev => prev || (last.weight_kg != null ? String(last.weight_kg) : ''))
      setDistance(prev => prev || (last.distance_m != null ? String(last.distance_m) : ''))
      setLastPerfHint(last)
    }
    prefill()
    return () => { cancelled = true }
  }, [exerciseId, onFetchLastPerformance])
  ...
  return (
    <div className="add-to-block">
      <style jsx>{`
        ...
        .last-perf-hint { color: rgba(255,255,255,.4); font-size: 10px; font-style: italic; }
      `}</style>
      ...
      {!open ? (
        ...
        <div className="add-to-block-form">
          ...
          {lastPerfHint && (
            <div className="last-perf-hint">↻ Pré-rempli avec ta dernière perf sur ce mouvement</div>
          )}
  ...
  return (
    <div className="editable-set-row">
      <style jsx>{`
        ...
        .set-1rm { color: #FDBA74; font-weight: 700; }
      ...
      {!editing ? (
        <div className="set-row">
          <div className="set-main">
            ...
            {log.weight_kg && log.reps > 1 && (
              <span className="set-1rm">≈1RM {estimate1RM(log.weight_kg, log.reps)}kg</span>
            )}
          </div>
...
export function MovementBlock({ blockExercise, blockType, onUpsertSetLog, onRemove, userId }) {
  ...
  const [persistentNote, setPersistentNote] = useState('')

  // Note persistante par mouvement (cf. ExerciseNoteModal) : suit le
  // mouvement, pas la séance. Chargement léger, silencieux en cas d'échec.
  useEffect(() => {
    let cancelled = false
    async function loadNote() {
      if (!userId || !blockExercise.exercise?.id) return
      const { data } = await supabase
        .from('personal_exercise_notes')
        .select('note')
        .eq('user_id', userId)
        .eq('exercise_id', blockExercise.exercise.id)
        .maybeSingle()
      if (!cancelled) setPersistentNote(data?.note || '')
    }
    loadNote()
    return () => { cancelled = true }
  }, [userId, blockExercise.exercise?.id])
  ...
  return (
    <div className="exercise-block">
      <style jsx>{`
        ...
        .exercise-note { margin: 4px 0 0; color: #FDBA74; font-size: 10.5px; font-style: italic; }
      ...
      <div className="exercise-head">
        <div>
          ...
          {persistentNote && <p className="exercise-note">📝 {persistentNote}</p>}
        </div>
        <div className="exercise-actions">
          ...
          <ExerciseActionsMenu
            ...
            userId={userId}
            lastWeightKg={blockExercise.logs?.[blockExercise.logs.length - 1]?.weight_kg ?? blockExercise.target_weight_kg}
            onNoteChange={setPersistentNote}
          />
...
export function SupersetGroup({ exercises, onUpsertSetLog, onRemoveExerciseFromBlock, userId }) {
          ...
          <div key={e.id} className="superset-ex-block">
            <div className="superset-ex-label">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <ExerciseActionsMenu exercise={e.exercise} userId={userId} />
...
export function EmomGroup({ exercises, onUpsertSetLog, onRemoveExerciseFromBlock, userId }) {
  return (
...
function EmomSimpleRow({ exercise, onUpsertSetLog, onRemove, userId }) {
      ...
      {!editing ? (
        <>
          ...
          <div className="actions">
            <ExerciseActionsMenu exercise={exercise.exercise} userId={userId} />
